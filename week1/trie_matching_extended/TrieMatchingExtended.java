...
public class TrieMatchingExtended implements Runnable {
	...
	List<Map<Character, Integer>> buildTrie(List<String> patterns, boolean[] isEnd) {
	...
	List <Integer> solve (String text, int n, List <String> patterns) {
		...
		boolean[] isEnd = new boolean[500000]; // maximum number of nodes
		List<Map<Character, Integer>> trie = buildTrie(patterns, isEnd);
		for (int i = 0; i < text.length(); i++) {
			int res = getPrefixTrieMatch(i, text, trie, isEnd);
			if (res != -1) {
				result.add(res);
			}
		}
	...
	int getPrefixTrieMatch (int startIndex, String text,
							List<Map<Character, Integer>> trie, boolean[] isEnd) {
		int index = startIndex;
		char symbol = text.charAt(index);
		int node = 0;
		while (true) {
			if (isLeaf(node, trie) || isEnd[node]) {
				return startIndex;
